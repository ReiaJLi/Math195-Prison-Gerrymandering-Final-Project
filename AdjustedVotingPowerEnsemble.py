# -*- coding: utf-8 -*-
"""
Created on Wed Dec 14 10:35:33 2022
...
# Get total adjusted population
tot_pop = 0
for v in az_bg_dg.nodes():
    tot_pop = tot_pop + az_bg_dg.nodes()[v]['adjtotal']
    ...
    updaters = {
        "our cut edges": cut_edges, #cut_edges is a built-in function in gerrychain 
         ...
         "district population": Tally("adjtotal", alias = "district population"),                           
                #adds up adjusted population for each district 
...
for part in our_random_walk: #a part is equivalent to one redistricting plan 
    ...
    for i in range(num_dist):
        adjvotingpower = 1/part["citizen voting age population"][i + 1] #measures the voting power in each district by taking 1/(citizen voting age population0 in that district
        adjvotingpowerlist.append(adjvotingpower)
        
    adjvotingpower_difference = max(adjvotingpowerlist) - min(adjvotingpowerlist) #measures the difference between the greatest and smallest voting power in the plan 
    adjvotingpower_ensemble.append(adjvotingpower_difference) #adds the difference to the ensemble 
    cutedge_ensemble.append(len(part["our cut edges"]))   
    
plt.figure()
plt.hist(cutedge_ensemble)
plt.title("AZ Adjusted Ensemble Number of Cutedges for 20,000 Plans")
...
plt.figure()
plt.hist(adjvotingpower_ensemble)
plt.title("AZ Adjusted Voting Power Ensemble for 20,000 Plans")
plt.xlabel("Greatest Difference in Voting Power")
plt.ylabel("Number of Plans")
plt.show()

#Now create a boxplot of the histogram 
plt.boxplot(adjvotingpower_ensemble)
plt.title("AZ Adjusted Voting Power Boxplot")
