# -*- coding: utf-8 -*-
"""
Created on Wed Dec 14 19:55:03 2022
...
#Opening the pickled dual graph 
...
#STEP 2: MAKE ENSEMBLE 

# Get total unadjusted population
tot_pop = 0
for v in new_dg.nodes():
    tot_pop = tot_pop + new_dg.nodes()[v]['total']
...
our_random_walk = MarkovChain(
    ...
    total_steps = 20000 #number of steps in the random walk 
    )

# What ensemble we want to build
hi_unadjvotingpower_ensemble = [] 
cutedge_ensemble = []

# This actually runs the random walk!
for part in our_random_walk: #a part is equivalent to one redistricting plan 
    hi_unadjvotingpowerlist = []
    for i in range(num_dist):
        hi_unadjvotingpower = 1/part["citizen voting age population"][i + 1] #measures the voting power in each district by taking 1/(citizen voting age population0 in that district
        hi_unadjvotingpowerlist.append(hi_unadjvotingpower)
        
    hi_unadjvotingpower_difference = max(hi_unadjvotingpowerlist) - min(hi_unadjvotingpowerlist) #measures the difference between the greatest and smallest voting power in the plan 
    hi_unadjvotingpower_ensemble.append(hi_unadjvotingpower_difference) #adds the difference to the ensemble 
    
    cutedge_ensemble.append(len(part["our cut edges"]))   
    
plt.figure()
plt.hist(cutedge_ensemble)
plt.title("HI Unadjusted Ensemble Number of Cutedges of 10,000 Plans")
...
plt.figure()
plt.hist(hi_unadjvotingpower_ensemble)
plt.title("HI Unadjusted Voting Power Ensemble of 10,000 Plans")
plt.xlabel("Greatest Difference in Voting Power")
plt.ylabel("Number of Plans")
plt.show()

#Now create a boxplot of the histogram 
plt.boxplot(hi_unadjvotingpower_ensemble)
plt.title("HI Unadjusted Voting Power Boxplot")
